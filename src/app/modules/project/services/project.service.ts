...
export class ProjectService {
  ...
  getAll(): Observable<Project[]> {
    ...
    .pipe(
     tap((data: void) => this.logService.log(`getAll Eseguito ${data}`)),
     retry(3)
     );
  ...
  add(project:Project): Observable<Project> {
    ...
    return this.httpClient.post<Project>('http://localhost:3000/projects', projectToAdd).pipe(
      tap((data: void) => this.logService.log(`add Eseguito ${data}`))
          );
  ...
  get(id: number): Observable<Project> {
    ...
    .pipe(
      tap((data) => this.logService.log('get Eseguito'))
      );
       
  }
}

function tap(arg0: () => void): import("rxjs").OperatorFunction<Project[], Project[]> {
  throw new Error('Function not implemented.');
}
function retry(arg0: number): import("rxjs").OperatorFunction<Project[], Project[]> {
  throw new Error('Function not implemented.');
}
